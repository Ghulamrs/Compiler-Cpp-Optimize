...
// **Dataflow problems over the flow graph**, after GCC's df: each problem
// is solved on request, over the blocks and edges the flow holds, and its
...
// **Reaching definitions**, one register at a time: the definitions of r
// that may hold its value on entry to each block. A definition is a number
// the caller chose; the caller says which definition each entry leaves in
// each register (`lastDef[k][r]`, or -1), and which definition stands for
// what a block entered from nowhere this function shows starts with
// (`unknownIn[b][r]`, or -1).
struct ReachingDefs {
