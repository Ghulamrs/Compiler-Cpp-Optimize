...
ExprPtr Parser::comparison(BinOp op, ExprPtr lhs, ExprPtr rhs, std::size_t pos) {
    ...
    if (null || lhs->type()->isPointer() || rhs->type()->isPointer() ||
        ...
        // **A pointer beside an integer: only a null pointer constant**, [expr.eq] and
        // [expr.rel]; `p == 1` was accepted here (cl C2446, the review's A26).
...
ExprPtr Parser::primary(Program *program) {
    ...
    if (peek().kind == TokenKind::Ident) {
        ...
        // Also the class's own body, `static const int n = 8;` as an array bound two lines on.
        const Type *ownScope = currentClass_ != nullptr ? currentClass_
                             : classStack_.empty() ? nullptr : classStack_.back();
        const Type *staticScopes[2] = { ownScope, lambdaScope() };
